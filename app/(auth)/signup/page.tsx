      ...
      <div className="container relative py-8 sm:py-12 lg:py-16">
        ...
        <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-6 items-stretch">
          <section className="hidden lg:flex border-gradient rounded-3xl bg-gradient-card shadow-card p-10 flex-col justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-primary mb-4">Create your account</p>
              <h1 className="font-display text-4xl font-semibold tracking-tight leading-tight text-gradient max-w-lg">
                Start solving business problems with the right AI stack.
              </h1>
              <p className="mt-4 text-muted-foreground max-w-md">
                Join teams that discover the best tools, experts, and governance paths from one command center.
              </p>
            </div>
            <ul className="space-y-3 text-sm mt-10">
              {[
                "Ranked model and tool recommendations",
                "Pre-vetted talent marketplace",
                "Built-in compliance and cost tracking",
              ].map((item) => (
          ...
          <section className="border-gradient rounded-3xl glass shadow-card p-6 sm:p-8 lg:p-9">
            ...
            <p className="text-sm text-muted-foreground mt-2 mb-6">Set up your 1AIBase workspace</p>
            ...
            <div className="mt-6 flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
              <span className="inline-flex items-center gap-1.5"><ShieldCheck className="size-3.5 text-primary" /> Encrypted auth and provider sign-in</span>
              <Link href="/login" className="hover:text-foreground transition-colors">Already registered?</Link>
            </div>
